...
function stampaEtichette(albero) {
    ...
    return valSottoalberi + albero.val;
}


stampaEtichette(albero);


function stampaEtichette2(albero) {
    if (albero === undefined) {
        return 0;
    }
    const sx = stampaEtichette2(albero.sx);
    const dx = stampaEtichette2(albero.dx);
    if ((albero.val < sx) || (albero.val < dx)) {
        console.log(albero.val);
    }
    return sx + dx + albero.val
}

stampaEtichette2(albero)
